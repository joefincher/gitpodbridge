(function ($, Drupal, drupalSettings) {
  ...
  $(document).ready(function() {
    ...
    if (settings.enabled) {
      ...
      else if (settings.language_detection == 'geoip_db' || settings.language_detection == '_server') {
        // Check geoip.
        var browser_lang = '';
        var xhr = $.ajax({
          type: "GET",
    ...
    function languageSuggestionRoutine() {
      ...
      if (redrectLang !== undefined && settings.always_redirect && redrectLangCode !== undefined && redrectLangCode !== current_lang) {
        window.location.href = redrectLang;
      }
      ...
      for (var langcode in settings.mapping) {
        if (settings.mapping.hasOwnProperty(langcode)) {
          ...
          var codeArr = (langObject.browser_lang.indexOf(',') > -1) ? langObject.browser_lang.split(',') : [langObject.browser_lang];
          var index = 0;

          while (index < codeArr.length) {
            if (langcode != current_lang && codeArr[index] == browser_lang.toLowerCase()) {
